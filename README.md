...
This EKS platform includes a production-ready monitoring stack deployed using the **kube-prometheus-stack** Helm chart. It provides real-time cluster visibility, historical metrics, dashboards, and alerting.

## Components

### Metrics Server

A lightweight metrics aggregator that collects **CPU and Memory** usage from Kubernetes nodes and pods via the kubelet. It powers:

* `kubectl top`
* Horizontal Pod Autoscaler (HPA)
* Vertical Pod Autoscaler (VPA)

It stores metrics only in memory and is **not** intended for historical monitoring.

---

### Prometheus

Prometheus is the central monitoring system. It periodically scrapes metrics from Kubernetes components, stores them as time-series data, and enables powerful querying using PromQL.
...
Grafana provides dashboards for visualizing metrics stored in Prometheus. It does not collect metrics itself but queries Prometheus to display real-time and historical data.

---

### Alertmanager

Alertmanager processes alerts generated by Prometheus and routes notifications to channels such as Email, Slack, Microsoft Teams, or PagerDuty.

---

### Node Exporter

Runs on every worker node as a DaemonSet and exposes operating system metrics, including:
...
Exports the state of Kubernetes resources such as Deployments, Pods, Nodes, Services, StatefulSets, and PersistentVolumeClaims. These metrics help monitor the health and status of cluster objects.
...
| ---------------------------- | ------------------------------- |
| Current CPU & Memory metrics | Historical metrics storage      |
| Powers HPA/VPA               | Monitoring & Alerting           |
| Supports `kubectl top`       | Dashboards via Grafana          |
| In-memory only               | Persistent time-series database |

Together, these components provide a complete observability solution, enabling resource monitoring, performance analysis, alerting, and troubleshooting for the EKS cluster.
